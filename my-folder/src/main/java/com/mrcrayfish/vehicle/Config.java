        ...
        Client(ForgeConfigSpec.Builder builder)
        {
            builder.comment("Client configuration settings").push("client");
            {
                builder.comment("Configuration options for debugging vehicles").push("debug");
                {
                    ...
                    this.renderDebugging = builder.comment("If true, renders lines to help visualise steering direction and target position.")
                        .define("renderSteeringDebug", false);
                    ...
                    this.reloadVehiclePropertiesEachTick = builder.comment("If true, the vehicle properties will be reloaded each tick.")
                        .define("reloadVehiclePropertiesEachTick", false);
                    this.forceRenderAllInteractableBoxes = builder.comment("If true, when rendering debug outlines all interactables boxes will be rendered rather than just the active")
                        .define("forceRenderAllInteractableBoxes", false);
                    this.debugCamera = builder.comment("Enables use of debug camera tools")
                ...
                builder.comment("Configuration for display related options").push("display");
                {
                    ...
                    this.forceFirstPersonOnExit = builder.comment("If enabled, camera perspective will always be forced back to first person when exiting a vehicle.")
                        .define("forceFirstPersonOnExit", false);
                    ...
                    this.hoseSegments = builder.comment("The amount of segments to use to render the hose on a gas pump. The lower the value, the better the performance but renders a less realistically looking hose")
                        .defineInRange("hoseSegments", 16, 1, 100);
                    ...
                    builder.comment("Configuration for camera related options").push("camera");
                    {
                        this.immersiveCamera = builder.comment("If true, uses an improved camera system when riding vehicles. Disabling this option will restore the default camera but it will break the experience of some vehicles. If you do disable this, clearly you don't care about the weeks MrCrayfish spent developing this awesome new camera system and want to hurt your eyes, you've been warned!")
                            .define("immersiveCamera", true);
                        this.followVehicleOrientation = builder.comment("Makes the camera follow the vehicles traveling direction and rotations. This will be limited to yaw if immersiveCamera is disabled.")
                            .define("followVehicleOrientation", true);
                        this.useVehicleAsFocusPoint = builder.comment("In third person, uses the vehicle as the focus point rather than the players head. This only has an effect when immersiveCamera is enabled.")
                            .define("useVehicleAsFocusPoint", true);
                        this.shouldFollowPitch = builder.comment("Makes the camera follow vehicle rotations on the x-axis (pitch). This only has an effect when followVehicleOrientation is enabled.")
                            .define("shouldFollowPitch", true);
                        this.shouldFollowYaw = builder.comment("Makes the camera follow vehicle rotations on the y-axis (yaw). This only has an effect when followVehicleOrientation is enabled.")
                            .define("shouldFollowYaw", true);
                        this.shouldFollowRoll = builder.comment("Makes the camera follow vehicle rotations on the z-axis (roll). This only has an effect when followVehicleOrientation is enabled.")
                            .define("shouldFollowRoll", true);
                    }
                    builder.pop();
                }
                builder.pop();
    ...
    public static class Server
    {
        ...
        Server(ForgeConfigSpec.Builder builder)
        {
            builder.comment("Server configuration settings").push("common");
            {
                ...
                this.energyConsumptionFactor = builder.comment("Change the amount of fuel vehicles consumes by multiplying the consumption rate by this factor")
                    .defineInRange("fuelConsumptionModifier", 1.0, 0.0, Double.MAX_VALUE);
                this.disabledVehicles = builder.comment("A list of vehicles that are prevented from being crafted in the workstation")
                    .defineList("disabledVehicles", List.of(), o -> true);
                this.validFuels = builder.comment("A list of fluids that can be used as fuel for vehicles")
                    .defineList("validFuels", List.of("vehicle:fuelium", "immersiveengineering:biodiesel", "immersivepetroleum:diesel"), o -> true);
                this.globalSpeedLimit = builder.comment("The maximum speed (in blocks per second) vehicles are allowed to travel. This will prevent vehicles travelling faster than the specified amount")
                    .defineInRange("globalSpeedLimit", 100.0, 0.0, 100.0);
                builder.pop();
                ...
                builder.comment("Configuration options for blocks").push("blocks");
                {
                    ...
                    this.maxHoseDistance = builder.comment("The maximum distance before the hose from the gas pump or fluid hose breaks")
                        .defineInRange("maxHoseDistance", 10.0, 1.0, 100.0);
                    this.gasPumpCapacity = builder.comment("The fluid capacity of the gas pump in millibuckets")
                        .defineInRange("gasPumpCapacity", 50000, 1, Integer.MAX_VALUE);
                    builder.pop();
                    ...
                    this.pumpCapacity = builder.comment("The fluid capacity of the fluid pump in millibuckets")
                        .defineInRange("pumpCapacity", 500, 1, Integer.MAX_VALUE);
                    builder.pop();

                    builder.comment("Configuration options for fluid extractors").push("fluid_extractor");
                    this.extractorExtractTime = builder.comment("The amount of ticks before fluid is extracted from an item")
                        .defineInRange("extractorExtractTime", 600, 1, Integer.MAX_VALUE);
                    this.extractorCapacity = builder.comment("The fluid capacity of the fluid extractor in millibuckets")
                        .defineInRange("extractorCapacity", 5000, 1, Integer.MAX_VALUE);
                    builder.pop();

                    builder.comment("Configuration options for fluid mixers").push("fluid_mixer");
                    this.mixerMixTime = builder.comment("The amount of ticks to mix fluids together")
                        .defineInRange("mixerMixTime", 100, 1, Integer.MAX_VALUE);
                    this.mixerInputCapacity = builder.comment("The input fluid capacity of the fluid mixer in millibuckets")
                        .defineInRange("mixerInputCapacity", 5000, 1, Integer.MAX_VALUE);
                    this.mixerOutputCapacity = builder.comment("The output fluid capacity of the fluid mixer in millibuckets")
                        .defineInRange("mixerOutputCapacity", 10000, 1, Integer.MAX_VALUE);
                    builder.pop();

                    builder.comment("Configuration options for fuel drums").push("fuel_drum");
                    this.fuelDrumCapacity = builder.comment("The fluid capacity of the fuel drum in millibuckets")
                        .defineInRange("fuelDrumCapacity", 40000, 1, Integer.MAX_VALUE);
                    this.industrialFuelDrumCapacity = builder.comment("The fluid capacity of the industrial fuel drum in millibuckets")
                        .defineInRange("industrialFuelDrumCapacity", 75000, 1, Integer.MAX_VALUE);
                    builder.pop();
                }
                builder.pop();
                ...
                builder.comment("Configuration options for items").push("items");
                {
                    ...
                    this.jerryCanCapacity = builder.comment("The fluid capacity of the jerry can in millibuckets")
                        .defineInRange("jerryCanCapacity", 5000, 1, Integer.MAX_VALUE);
                    this.industrialJerryCanCapacity = builder.comment("The fluid capacity of the industrial jerry can in millibuckets")
                        .defineInRange("industrialJerryCanCapacity", 15000, 1, Integer.MAX_VALUE);
                    this.jerryCanFillRate = builder.comment("The amount of fluid transferred when pouring or filling a jerry can")
                        .defineInRange("fillRate", 500, 1, Integer.MAX_VALUE);
                    builder.pop();

                    builder.comment("Configuration options for spray cans").push("spray_can");
                    this.sprayCanCapacity = builder.comment("The amount of sprays before a spray can becomes empty")
                        .defineInRange("sprayCanCapacity", 20, 1, Integer.MAX_VALUE);
                    builder.pop();
                }
                builder.pop();
            }
